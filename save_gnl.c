...
//check if '\n' '\0' (last nl char)
//len of buf is smaller than buf_size
size_t	check_end_nl(char *buf)
{
	if (check_pos_nl(buf) == 0 && ft_strlen(buf) == 1)
		return (1);
	return (0);
}
...
static ssize_t	extract_line(char **line, char *buf)
{
	...
	//printf("nl pos: %zu || ", new_line);
	*line = ft_strjoin_gnl(*line, buf, new_line);
	if (!line)
		return (-1);
	if (new_line == BUFFER_SIZE && !(check_end_nl(buf)))
	{
		move_chars(buf, (new_line));
		return (0);
	}
	else
	{
